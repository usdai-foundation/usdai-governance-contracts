...
contract DeployOmnichainEnvironment is Deployer {
    ...
    function run(
        ...
        bytes memory oadapterChipCalldata = abi.encodeWithSelector(
            ...
            OADAPTER_CHIP_SALT,
            abi.encodePacked(type(OLockAdapter).creationCode, abi.encode(CHIP_ADDRESS, lzEndpoint, msg.sender))
        );
        ...
        bytes memory oadapterStakedChipCalldata = abi.encodeWithSelector(
            ...
            abi.encodePacked(type(OLockAdapter).creationCode, abi.encode(STAKED_CHIP_ADDRESS, lzEndpoint, msg.sender))
        );
